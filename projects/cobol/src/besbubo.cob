      ...
      *> command-line flag stuffs
       ...
       PROCEDURE DIVISION.
           ...
           *> print row helper procedure, should you want to use it
           *> (you do not have to!)
           print-schedule-row.
           MOVE WS-IDX TO WS-JDX.
           SUBTRACT 1 FROM WS-IDX.
           MULTIPLY 100 BY WS-IDX.
           ADD 1600 TO WS-IDX.
           DIVIDE 2 INTO WS-IDX.
           DIVIDE WS-JDX BY 2 GIVING WS-Q REMAINDER WS-R.
           IF WS-R = 0 THEN
               SUBTRACT 20 FROM WS-IDX
           END-IF.
           DISPLAY WS-IDX WITH NO ADVANCING.
           MOVE WS-JDX TO WS-IDX.
           MOVE 1 TO WS-JDX.
           PERFORM UNTIL WS-JDX > WS-NUM-DAYS
               DISPLAY " | " WS-DAYS(WS-IDX, WS-JDX) WITH NO ADVANCING
               ADD 1 TO WS-JDX
           END-PERFORM.
           DISPLAY " ".
           ADD 1 TO WS-IDX.
